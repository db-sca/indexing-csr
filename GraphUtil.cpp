...
void GraphUtil::dfs_backwards(Graph &g, int vid, vector<int> &postorder, vector<bool> &visited) {
	visited[vid] = true;
	...
	for (eit = el.begin(); eit != el.end(); eit++) {
		if (!visited[*eit]) {
			nextid = *eit;
			dfs_backwards(g, nextid, postorder, visited);
		}
	}
	postorder.push_back(vid);
}
...
void GraphUtil::topological_sort_backwards(Graph &g, vector<int> &ts) {
	vector<bool> visited(g.num_vertices(), false);
	vector<int> postorder;
	vector<int> leaves = g.get_leaves();
	vector<int>::iterator sit;
	// depth-first-search whole graph
	for (sit = leaves.begin(); sit != leaves.end(); sit++){
		if (!visited[*sit]){
			dfs_backwards(g, (*sit), postorder, visited);
		}
	}
	ts = postorder;
}

void GraphUtil::topological_sort(Graph &g, vector<int> &re_ts, vector<int> &ts) {
	vector<bool> visited(g.num_vertices(), false);
	vector<int> preorder;
	vector<int> postorder;
	vector<int> roots = g.getRoots();
...
//void GraphUtil::treePathDecomposition(Graph tree, Graph& g, hash_map<int, list<int> >& pathMap) {
