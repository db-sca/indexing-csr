//
// Created by chaowyc on 19/3/2021.
//

#ifndef CFL_INDEXING_LOG_H
#define CFL_INDEXING_LOG_H
...
inline void initLogger(const string& folder, const string& method, const string& project, int query_num, TLogLevel level)
{
	string file_name = folder + "/" + method + "-" + project + "-" + to_string(query_num) + "-" + NowTime() + ".log";
	FILELog::ReportingLevel() = level;
	FILE* log_fd = fopen( file_name.c_str(), "w" );
	Output2FILE::Stream() = log_fd;
}
